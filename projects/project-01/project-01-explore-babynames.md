...
tbl_names_popular |> 
  ...
  labs(
    title = 'Top 5 Popular Names by Gender',
    subtitle = 'Between 2010 and 2020',
    caption = '',
    x = 'Number of Births',
    y = 'Name'
  ) +
...
tbl_names_by_letter
# Math checks out
sum(tbl_names_by_letter$pct_births)
length(unique(tbl_names_by_letter$year))
```
...
``` r
glimpse(tbl_names_by_first_and_last_letter)
...
tbl_names_by_first_and_last_letter |> 
 ...
 labs(
   title = "First Letter vs. Last Letter Births",
   x = "First Letter",
   y = "Last Letter"
 ) +
...
tbl_names_vowel_consonant <- tbl_names |> 
  ...
  mutate(first_last = paste(first_letter_type, last_letter_type))
...
tbl_names_vowel_consonant |> 
  ...
  # Add labels (title, subtitle, caption, x, y)
  labs(
  title = "Vowel and Consonant Usage Trend in Names",
  subtitle = "",
  caption = "",
  x = "Year",
  y = "Percentage of Births"
